...
const getStatus = (v) => {
  if (v < 40)
    return {
      status: "건조",
      color: "#ff6b6b",
      desc: "토양이 건조한 상태입니다. 관수가 필요합니다.",
    };
  if (v <= 80)
    return {
      status: "적정",
      color: "#10b981",
      desc: "최적의 토양수분 상태입니다. 현재 상태를 유지하세요.",
    };
  return {
    status: "과습",
    color: "#f59e0b",
    desc: "토양이 과습 상태입니다. 배수 또는 관수 중단이 필요합니다.",
  };
};
...
export default function SoilMoistureGauge() {
  ...
  return (
    <div style={{ display: "flex", gap: 20, alignItems: "center", height: "100%" }}>
      ...
      <div style={{ flex: 1 }}>
        <h3 style={{ margin: 0 }}>토양 수분 상태</h3>
        <p style={{ margin: "6px 0 0 0", color: "#6b7280" }}>
          현재 토양수분: <strong style={{ color: status.color }}>{currentPercent}%</strong>
        </p>

        <div style={{ marginTop: 10, display: "flex", gap: 8 }}>
          <div style={{ padding: 8, background: "#fdecea", borderRadius: 8, flex: 1 }}>
            <strong style={{ color: "#b91c1c" }}>건조 (0-40%)</strong>
            <div style={{ fontSize: 13, color: "#6b7280" }}>즉시 관수가 필요합니다.</div>
          </div>
          ...
          <div style={{ padding: 8, background: "#fff7ed", borderRadius: 8, flex: 1 }}>
            ...
            <div style={{ fontSize: 13, color: "#6b7280" }}>배수 또는 관수 중단이 필요합니다.</div>
          </div>
        </div>
