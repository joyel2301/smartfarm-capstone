...
import { fetchGrowthWeekly, upsertGrowthWeekly } from '../api/growthApi';
import StrawberryGrowthChart from '../../weather/components/StrawberryGrowthChart';

export default function GrowthDataPanel({ onSaved }) {
  ...
  const load = async () => {
    ...
    setErr('');
    try {
      const rows = await fetchGrowthWeekly();
      setData(rows);
    } catch (e) {
    ...
    <div style={{ display: 'flex', gap: 16, height: '100%' }}>
      <div style={{ flex: 2, minWidth: 0 }}>
        {loading ? (
          ...
          // StrawberryGrowthChart expects: [{week, height, leaves}]
          <StrawberryGrowthChart data={data} />
        )}
      </div>

      <div style={{ flex: 1 }}>
        <h3 style={{ margin: '0 0 8px 0' }}>생육 데이터 추가</h3>
